...
class SessionNotResumableError(NexusCoreError):
    """Raised when a session holds no provider checkpoint a backend could resume."""

    code = "session_not_resumable"

    def __init__(self, session_id: str) -> None:
        self.session_id = session_id
        super().__init__(f"Session {session_id} has no provider checkpoint to resume")
